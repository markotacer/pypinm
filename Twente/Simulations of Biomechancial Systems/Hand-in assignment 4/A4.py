...
def segdynshell(t,state,parms):
   ...
   """
    ...
    V=np.array(
        [np.nan, np.nan, np.nan, # Fx nseg+1 horizontal joint reaction forces
         np.nan, np.nan, np.nan, # Fy nseg+1 vertical joint reaction forces
         0, 0, 0,# M nseg+1 net joint moments
         0, 0,  # Fextx nseg horizontal external forces
         0, 0,  # Fexty nseg vertical externalforces
         np.nan, np.nan, # Mext nseg external moments
